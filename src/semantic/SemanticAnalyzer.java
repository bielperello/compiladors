...
import ast.instructions.AssignNode;
import ast.instructions.InputNode;
import ast.instructions.InstrNode;
import ast.instructions.OutputNode;
import ast.instructions.conditionals.CaseNode;
import ast.instructions.conditionals.CondNode;
import ast.instructions.conditionals.IfNode;
import ast.instructions.conditionals.SwitchNode;
import ast.instructions.loops.DoWhileNode;
import ast.instructions.loops.LoopNode;
import ast.instructions.loops.WhileNode;
import semantic.symbols.Simbol;
import semantic.symbols.SymbolTable;
import semantic.symbols.TipusUtils;
import semantic.symbols.descripcio.*;
...
public class SemanticAnalyzer {
    ...
    public String getSymbolTableDump() {
        StringBuilder sb = new StringBuilder();
        sb.append("--- Taula de Símbols ---\n");

        // 1) Global
        sb.append(globalScope.dumpScope());

        // 2) La resta d’àmbits tancats
        List<SymbolTable> tables = new ArrayList<>(SymbolTable.getAllTables());
        tables.sort(Comparator.comparingInt(SymbolTable::getScopeId));

        for (SymbolTable t : tables) {
            // Evitar duplicar el global
            if (t.getParent() == null) continue;
            sb.append(t.dumpScope());
        }
