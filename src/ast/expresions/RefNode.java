...
public class RefNode extends ExprNode {
    ...
    public void generateCode() {
        ...
        boolean isLogic = (this.getKind() == Kind.LOGIC);
        if (isLogic && this.mode == ModeExpr.MODERESULT) {
            // E -> (v != 0) ? true : false
            int zero = CodeGenerator.novaVarTemporal();
            CodeGenerator.genera(OpCode.COPY, "0", zero);

            // 1) salt condicionat a CERT (backpatch)
            CodeGenerator.genera(OpCode.IF_NE, this.resultVar, zero, CodeGenerator.NUL_VAL);
            int posTrue = CodeGenerator.pc();

            // 2) salt incondicional a FALS (backpatch)
            CodeGenerator.genera(OpCode.GOTO, CodeGenerator.NUL_VAL);
            int posFalse = CodeGenerator.pc();

            this.trueList = java.util.List.of(posTrue);
            this.falseList = java.util.List.of(posFalse);
        }
    }
